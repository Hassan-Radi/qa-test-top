...
public enum ExecutionMode implements IExecutionMode {
	...
	LOCAL() {
		...
		public WebDriver getFirefoxDriver() {
			System.setProperty(TestData.GECKO_DRIVER_LOCAL_PATH_PROPERTY, TestData.GECKO_DRIVER_LOCAL_PATH);
			// To stop seeing the GeckoDriver annoying logs
			System.setProperty(FirefoxDriver.SystemProperty.BROWSER_LOGFILE, TestData.BROWSER_LOG_FILE_PATH);
		...
		public WebDriver getChromeDriver() {
			System.setProperty(TestData.CHROME_DRIVER_DRIVER_LOCAL_PATH_PROPERTY,
					TestData.CHROME_DRIVER_DRIVER_LOCAL_PATH);
	...
	BROWSERSTACK() {
		...
		public WebDriver getFirefoxDriver() {
			...
			/**
			 * TODO: Move this to an external file to allow better visibility and make it easier to
			 * change the values or add new ones without re-compiling the code.
			 */
			capabilities.setCapability(TestData.BROWSER_NAME_W3C_CAPABILITY, TestData.FIREFOX_BROWSER);
			capabilities.setCapability(TestData.BROWSER_VERSION_W3C_CAPABILITY, TestData.FIREFOX_BROWSER_VERSION);
			...
			capabilities.setCapability(TestData.SELENIUM_VERSION_CAPABILITY, TestData.SELENIUM_VERSION_VALUE);
			capabilities.setCapability(TestData.GECKO_DRIVER_VERSION_CAPABILITY, TestData.GECKO_DRIVER_VERSION_VALUE);
		...
		public WebDriver getChromeDriver() {
			...
			capabilities.setCapability(TestData.BROWSER_VERSION_CAPABILITY, TestData.CHROME_BROWSER_VERSION);
			capabilities.setCapability(TestData.OS_CAPABILITY, TestData.WINDOWS_OS_NAME);
			capabilities.setCapability(TestData.OS_VERSION_CAPABILITY, TestData.WINDOWS_10_NAME);
			capabilities.setCapability(TestData.RESOLUTION_CAPABILITY, TestData.RESOLUTION_VALUE);
			capabilities.setCapability(TestData.ACCEPT_SSL_ALERTS_CAPABILITY, TestData.TRUE_VALUE);
			capabilities.setCapability(TestData.SELENIUM_VERSION_CAPABILITY, TestData.SELENIUM_VERSION_VALUE);
			capabilities.setCapability(TestData.CHROME_DRIVER_VERSION_CAPABILITY, TestData.CHROME_DRIVER_VERSION_VALUE);
			capabilities.setCapability(TestData.PROJECT_CAPABILITY, TestData.PROJECT_VALUE);
		 ...
		 * 
		 * @param capabilities
		 *            The capabilities to use when creating the remote WebDriver instance.
		...
		private WebDriver createBrowserStackRemoteDriver(DesiredCapabilities capabilities) {
			try {
				return new RemoteWebDriver(new URL(
						String.format(TestData.BROWSERSTACK_HUB_URL, TestData.BROWSERSTACK_USER_ENVIRONEMNT_VARIABLE,
								TestData.BROWSERSTACK_TOKEN_ENVIRONEMNT_VARIABLE)),
						capabilities);
			} catch (Exception e) {
				throw new RuntimeException("Failed to initialise the remote driver instance. Exception = " + e);
			}
		}
	};
